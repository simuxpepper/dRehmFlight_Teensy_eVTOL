...
//RP2040 I2C Slave Code

float roll_IMU, pitch_IMU, yaw_IMU;
unsigned long channel_1_pwm, channel_2_pwm, channel_3_pwm, channel_4_pwm, channel_5_pwm, channel_6_pwm;

#include "src/Teensy4_I2C/i2c_driver.h"
#include "src/Teensy4_I2C/i2c_driver_wire.h"
#define SLAVE_ADDRESS 0x6C

void RP2040_Setup();
void RP2040_Request_Loop();

void RP2040_Setup() {
  Wire.begin(SLAVE_ADDRESS);
  Wire.onRequest(RP2040_Request_Loop);
  Wire.setClock(400000);
}

void write_2byte(int16_t data);

void write_2byte(int16_t data) {
  Wire.write((uint8_t) (((int16_t) data & 0xFF00) >> 8));
  Wire.write((uint8_t) ((int16_t) data & 0x00FF));
}

void RP2040_Request_Loop() {
  write_2byte((int16_t) roll_IMU);
  write_2byte((int16_t) pitch_IMU);
  write_2byte((int16_t) yaw_IMU);
  
  write_2byte((int16_t) channel_1_pwm);
  write_2byte((int16_t) channel_2_pwm);
  write_2byte((int16_t) channel_3_pwm);
  write_2byte((int16_t) channel_4_pwm);
  write_2byte((int16_t) channel_5_pwm);
  write_2byte((int16_t) channel_6_pwm);
  
  // Serial.print(F("Roll: "));
  // Serial.print((int16_t) roll_IMU);
  // Serial.print(F(", Pitch: "));
  // Serial.print((int16_t) pitch_IMU);
  // Serial.print(F(", Yaw: "));
  // Serial.println((int16_t) yaw_IMU);
}
...
float GyroErrorX = -2.37;
float GyroErrorY = 0.66;
float GyroErrorZ = 1.24;
...
const int servo5Pin = 14;
const int servo6Pin = 15;
const int servo7Pin = 22;
const int servo8Pin = 23;
const int servo9Pin = 29;
const int servo10Pin = 33;
const int servo11Pin = 36;
const int servo12Pin = 37;
...
//Radio communication:
//unsigned long channel_1_pwm, channel_2_pwm, channel_3_pwm, channel_4_pwm, channel_5_pwm, channel_6_pwm; //Uncomment if not using RP2040
...
//float roll_IMU, pitch_IMU, yaw_IMU; //Uncomment if not using RP2040
...
float m7_command_scaled, m8_command_scaled, m9_command_scaled, m10_command_scaled, m11_command_scaled, m12_command_scaled;
int m1_command_PWM, m2_command_PWM, m3_command_PWM, m4_command_PWM, m5_command_PWM, m6_command_PWM;
int m7_command_PWM, m8_command_PWM, m9_command_PWM, m10_command_PWM, m11_command_PWM, m12_command_PWM;
float s1_command_scaled, s2_command_scaled, s3_command_scaled, s4_command_scaled, s5_command_scaled, s6_command_scaled, s7_command_scaled;
float s8_command_scaled, s9_command_scaled, s10_command_scaled, s11_command_scaled, s12_command_scaled;
int s1_command_PWM, s2_command_PWM, s3_command_PWM, s4_command_PWM, s5_command_PWM, s6_command_PWM, s7_command_PWM;
int s8_command_PWM, s9_command_PWM, s10_command_PWM, s11_command_PWM, s12_command_PWM;
...
void setup() {
  ...
  pinMode(m8Pin, OUTPUT);
  pinMode(m9Pin, OUTPUT);
  pinMode(m10Pin, OUTPUT);
...
float forward_mode() {
  if (channel_6_pwm < 1400) return 1;
  else if (channel_6_pwm < 1600) return 0.5;
  return 0;
}
float hover_mode() {
  if (channel_6_pwm > 1600) return 1;
  else if (channel_6_pwm > 1400) return 0.5;
  return 0;
}
float transition_mode() {
  if (forward_mode() == 0.5) return 1;
  return 0;
}
...
void controlMixer() {
  ...
  //Quad mixing - EXAMPLE
  m3_command_scaled = hover_mode() * (thro_des - pitch_PID + roll_PID + 0.8 * yaw_PID);  //Front Left
  m1_command_scaled = hover_mode() * (thro_des - pitch_PID - roll_PID - 0.8 * yaw_PID);  //Front Right
  m2_command_scaled = hover_mode() * (thro_des + pitch_PID - roll_PID + 0.8 * yaw_PID);  //Back Right
  m4_command_scaled = hover_mode() * (thro_des + pitch_PID + roll_PID - 0.8 * yaw_PID);  //Back Left
  m5_command_scaled = forward_mode() * thro_des;
  m6_command_scaled = 0;
  m7_command_scaled = 0;
  m8_command_scaled = 0;
  m9_command_scaled = 0;
  m10_command_scaled = 0;
  m11_command_scaled = 0;
  m12_command_scaled = 0;

  //0.5 is centered servo, 0.0 is zero throttle if connecting to ESC for conventional PWM, 1.0 is max throttle
  s1_command_scaled = roll_passthru;
  s2_command_scaled = 0.5 - roll_passthru;
  s3_command_scaled = yaw_passthru;
  s4_command_scaled = 0.5 + yaw_passthru;
  s5_command_scaled = 0.5 + pitch_passthru;
  s6_command_scaled = 0;
  s7_command_scaled = 0;
  s8_command_scaled = 0;
  s9_command_scaled = 0;
  s10_command_scaled = 0;
...
void scaleCommands() {
  ...
  m8_command_PWM = m8_command_scaled * 125 + 125;
  m9_command_PWM = m9_command_scaled * 125 + 125;
  m10_command_PWM = m10_command_scaled * 125 + 125;
  m11_command_PWM = m11_command_scaled * 125 + 125;
  m12_command_PWM = m12_command_scaled * 125 + 125;
  ...
  m8_command_PWM = constrain(m8_command_PWM, 125, 250);
  m9_command_PWM = constrain(m9_command_PWM, 125, 250);
  m10_command_PWM = constrain(m10_command_PWM, 125, 250);
  m11_command_PWM = constrain(m11_command_PWM, 125, 250);
  m12_command_PWM = constrain(m12_command_PWM, 125, 250);
...
void commandMotors() {
  ...
  digitalWrite(m9Pin, HIGH);
  digitalWrite(m10Pin, HIGH);
  digitalWrite(m11Pin, HIGH);
  digitalWrite(m12Pin, HIGH);
  pulseStart = micros();

  //Write each motor pin low as correct pulse length is reached
  while (wentLow < 12) {  //Keep going until final (6th) pulse is finished, then done
    timer = micros();
    ...
    if ((m7_command_PWM <= timer - pulseStart) && (flagM7 == 0)) {
      digitalWrite(m7Pin, LOW);
      wentLow = wentLow + 1;
      flagM7 = 1;
    }
    if ((m8_command_PWM <= timer - pulseStart) && (flagM8 == 0)) {
      digitalWrite(m8Pin, LOW);
      wentLow = wentLow + 1;
      flagM8 = 1;
    }
    if ((m9_command_PWM <= timer - pulseStart) && (flagM9 == 0)) {
      digitalWrite(m9Pin, LOW);
      wentLow = wentLow + 1;
      flagM9 = 1;
    }
    if ((m10_command_PWM <= timer - pulseStart) && (flagM10 == 0)) {
      digitalWrite(m10Pin, LOW);
      wentLow = wentLow + 1;
      flagM10 = 1;
    }
    if ((m11_command_PWM <= timer - pulseStart) && (flagM11 == 0)) {
      digitalWrite(m11Pin, LOW);
      wentLow = wentLow + 1;
      flagM11 = 1;
    }
    if ((m12_command_PWM <= timer - pulseStart) && (flagM12 == 0)) {
...
void calibrateESCs() {
  ...
  while (true) {
    ...
    m7_command_scaled = thro_des;
    m8_command_scaled = thro_des;
    m9_command_scaled = thro_des;
    m10_command_scaled = thro_des;
    m11_command_scaled = thro_des;
    m12_command_scaled = thro_des;
    ...
    servo6.write(s6_command_PWM);
    servo7.write(s7_command_PWM);
    servo3.write(s8_command_PWM);
    servo4.write(s9_command_PWM);
    servo5.write(s10_command_PWM);
    servo6.write(s11_command_PWM);
    servo7.write(s12_command_PWM);
    commandMotors();  //Sends command pulses to each motor pin using OneShot125 protocol
...
void printMotorCommands() {
  if (current_time - print_counter > 10000) {
    ...
    Serial.print(F(" m7_command:"));
    Serial.print(m7_command_PWM);
    Serial.print(F(" m8_command:"));
    Serial.print(m8_command_PWM);
    Serial.print(F(" m9_command:"));
    Serial.print(m9_command_PWM);
    Serial.print(F(" m10_command:"));
    Serial.print(m10_command_PWM);
    Serial.print(F(" m11_command:"));
    Serial.print(m11_command_PWM);
    Serial.print(F(" m12_command:"));
    Serial.println(m12_command_PWM);
  }
}
